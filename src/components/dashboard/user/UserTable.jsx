...
export default function UserTable({ user }) {
  ...
  const addUser = async (data) => {
    try {
      const response = await createUser(user.accessToken, {
        username: data.email,
        role: data.role,
  ...
  return (
    <Card>
      <CardContent>
              ...
              <TableBody>
                {
                  users.map((userItem, index) => (
                    <TableRow key={userItem._id}>
                      ...
                      <TableCell>
                        {
                          (userItem.email !== user.email) && (<Button
                            color="secondary"
                            title="Delete user"
        ...
        {
          users.length === 0 && <Typography align="center" style={{marginTop: '20px'}}>No se han podid11o cargar los usuarios</Typography>
        }
      </CardContent>
    </Card>
  )
